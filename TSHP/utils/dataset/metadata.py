        ...
        try: # try identify transcript from audiopath
            ...
            try: # try identify transcript from audio filename
                ...
                try: # try identify transcript from audio basename
                    ...
                    raise FileNotFoundError(f'"{audio_file}" not found in filelists')
        try:
...
def clipper_naming_exceptions(audio_file, source, source_type, voice):
    ...
    if "Other sources/Star Trek (John de Lancie, Discord)" in audio_file:
        source_type, source, voice = "Show", "Star Trek", "Q"
    elif "Other sources/Eli, Elite Dangerous (John de Lancie, Discord)" in audio_file:
        source_type, source, voice = "Game", "Elite Dangerous", "Eli"
    elif "Other sources/A Little Bit Wicked (Kristin Chenoworth, Skystar)" in audio_file:
        source_type, source, voice = "Audiobook", "A Little Bit Wicked", voice
    elif "Other sources/Sum - Tales From the Afterlives (Emily Blunt, Tempest)" in audio_file:
        source_type, source, voice = "Audiobook", "Sum - Tales From the Afterlives", voice
    elif "Other sources/Dr. Who" in audio_file:
        source_type, source, voice = "Audiobook", "Dr. Who", voice
    elif "Other sources/Dan vs" in audio_file:
        source_type, source, voice = "Show", "Dan vs", voice
    elif "Other sources/Them's Fightin' Herds" in audio_file:
        source_type, source, voice = "Game", "Them's Fightin' Herds", voice
    elif "Other sources/CGP Grey" in audio_file:
        source_type, source, voice = "Show", "CGP Grey", voice
    elif "Other sources/ATHF" in audio_file:
        source_type, source, voice = "Show", "Aqua Teen Hunger Force", voice
    elif "/Songs" in audio_file:
        source_type, source, voice = "Music", source, voice+' Singing'
...
def get_dataset_meta(directory, meta=None, default_speaker=None, default_emotion=None, default_noise_level=None, default_source=None, default_source_type='audiobook',
                     audio_ext=None, audio_rejects=None, naming_system=None, skip_empty=False):
    """
    Looks for
     - audio paths
     - transcripts
    ...
    w.print0(f'\nReading "{directory}" for audio/text files')
    ...
    w.print0(f'Found {len(audio_files)} audio files.')
    ...
    w.print0(f'Found {len(txt_files)} text files.')
    ...
    w.print0(f'Identifying "{directory}" dataset type.')
    ...
    w.print0(f'"{directory}" identified as {dataset_style} style dataset.')
    ...
    for audio_file in audio_files:
        ...
        try:
            ...
            w.print1(f'{ex}\nSkipping file: "{audio_file}"')
            files_skipped+=1; continue
        except KeyError as ex:
            w.print1(f'{ex}\nSkipping file: "{audio_file}"')
            files_skipped+=1; continue
        if len(transcript) < 2:
            w.print1(f'Skipping file: "{audio_file}"')
            files_skipped+=1; continue
        if os.path.splitext(audio_file)[0] in audio_basepaths_added:# if same audio with another file extension was added already
            w.print1(f'Skipping file: "{audio_file}". An audio file with this basename has already been added.')
            files_skipped+=1; continue
    ...
    w.print1(f'Total: {len(audio_files)} audio paths found for Filelist\nAdded: {files_added} Files\nSkipped: {files_skipped} Files (either missing transcripts or file has duplicates)')
